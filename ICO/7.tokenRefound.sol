...
contract Crowdsale {
    ...
    function etherSearch () view returns (uint) {
        return etherSum;
    }

    function tokenSearch () view returns (uint) {
        return tokenReward.balanceOf(address(this));
    }

    function etherRefound () {
        beneficiary.transfer(etherSum);
        etherSum = 0;
    }

    function tokenRefound() {
        tokenReward.transfer(tokenAddress, tokenReward.balanceOf(address(this)));
    }
}
